...
export async function GET(req: NextRequest, {params}: {params: Promise<{slug: string}>}){
    ...
    const [post] = await sql`SELECT posts.id AS id, posts.title AS title, posts.content AS content, posts.created_at AS created_at, users.name AS author, users.image FROM posts JOIN users ON posts.user_id = users.id WHERE posts.slug = ${slug}`;
